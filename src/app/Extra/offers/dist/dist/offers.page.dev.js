...
function () {
  ...
  OffersPage.prototype.ngOnInit = function () {
    ...
    this.androidPermissions.checkPermission(this.androidPermissions.PERMISSION.READ_EXTERNAL_STORAGE).then(function (result) {
      return console.log('Has permission? read', result.hasPermission);
    }, function (err) {
      return _this.androidPermissions.requestPermission(_this.androidPermissions.PERMISSION.READ_EXTERNAL_STORAGE);
    }); // this.GetMyOffers();
  };
  ...
  OffersPage.prototype.GetMyOffers = function () {
    ...
    this.common.PostMethod("GetOffers", {
      b_id: new user_pipe_1.UserPipe().transform('b_id'),
      userid: localStorage.getItem("UserId"),
      usertype: new user_pipe_1.UserPipe().transform('usertype')
    }).then(function (res) {
      console.log(res);
  ...
  OffersPage.prototype.NotifiyMe = function () {
    ...
    this.common.PostMethod("NotifyMe", {
      b_id: new user_pipe_1.UserPipe().transform('b_id'),
      userid: localStorage.getItem("UserId"),
      type: 'Coupon'
    }).then(function (res) {
  ...
  OffersPage.prototype.downloadImage = function (ImageFullPath) {
    ...
    var RandNumber = Math.floor(Math.random() * (999 - 100 + 1) + 100); //   const url = ImageFullPath;
  ...
  OffersPage = __decorate([core_1.Component({
    selector: 'app-offers',
