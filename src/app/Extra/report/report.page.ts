...
import { CustomdatePage } from 'src/app/Modal/customdate/customdate.page';
@Component({
  selector: 'app-report',
...
export class ReportPage implements OnInit {
  ...
  async Customdateopen() {
    ...
    if (data.status) {
      ...
      this.common.PostMethod("GetAppReport", { userid: userid, b_id: new UserPipe().transform('b_id'), from: moment(data.fromdate).format('YYYY-MM-DD'), to: moment(data.todate).format('YYYY-MM-DD') }).then((res: any) => {
        this.lists = res.Data;
      });
    }
  }
